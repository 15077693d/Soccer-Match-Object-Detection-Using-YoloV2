...
def write_xml(folder, img, objects, tl, br, savedir):
    ...
    image = cv2.imread(str(img)) #load img from scandir
    ...
    for obj_name, topl, botr in zip(objects, tl, br):
        ...
        ET.SubElement(ob, 'pose').text = 'Unspecified' #can be left, right
    ...
    save_path = os.path.join(savedir, img.name.replace('png', 'xml')) #use image name
...
if __name__ == '__main__':
    folder = 'images'
    img = [path for path in sorted(Path('./images/Part1').glob('*.png'))][0]
